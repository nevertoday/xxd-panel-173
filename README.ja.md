...
本バッチには、基本的な比率とサムネイル検査を通過した生成サンプル（`assets/examples/sample-03.png`）を1点含みます。その他の再生成候補はレビュー待ちのため、公開サンプルには含めていません。原文・リンク・実行スクリプトを検証済みです。
...
紙と文字の微景観は10–20%、暖白余白は80–90%。約95%を象牙・米灰・紙褐に保ち、赤・金・青を足しません。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 173 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-173` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-173 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-173-prompt.en.md)／[中国語](references/xxd-panel-173-prompt.zh-CN.md)を参照してください。
