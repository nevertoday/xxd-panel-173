...
# XXD Panel 173｜상아색 종이와 문자 미니 풍경
...
이번 배치에는 기본 비율 및 썸네일 검사를 통과한 생성 샘플 1점(`assets/examples/sample-03.png`)이 포함됩니다. 나머지 재생성 후보는 검토 전이므로 공개 샘플에 포함하지 않았습니다. 원문·링크·실행 스크립트는 확인했습니다.
...
종이와 문자 미니 풍경은 10–20%, 따뜻한 흰 여백은 80–90%입니다. 약 95%를 상아·베이지 회색·종이 갈색으로 유지하고 빨강·금색·파랑을 넣지 않습니다.
...
- `design-only`: 전체 캔버스에 Panel 173의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-173`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-173 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-173-prompt.en.md)와 [중국어](references/xxd-panel-173-prompt.zh-CN.md)를 확인하세요.
