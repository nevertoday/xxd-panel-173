---
name: xxd-panel-173
description: "Create Panel 173 raster artwork: Ivory Paper-text Miniature Landscape. Use when the user invokes xxd-panel-173 or requests this specific visual language."
---
...
Compress the source into a tiny 10–20% paper-text landscape with 80–90% warm-white negative space. Use only essential book-page, upright paper, layered-paper or tiny narrative structures with very light paper shadows. Keep roughly 95% in ivory, beige-grey and paper-brown, with sparse warm ink-brown anchors. Do not add conspicuous red, gold, cyan or blue, heavy 3D, elaborate perspective or a complete illustrated scene.

Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-173/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-173-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
